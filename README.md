...
The command deliberately removes HyprDimension's saved state first; otherwise windows from an earlier run can fill the 2x4 grid. It selects a window on `DP-1` and switches to that window's current workspace before assigning and running the test. Assignment only updates plugin state at present; it does not move or transform live windows yet.
